...
function SpotlightCard({ ads = [], showToast, setActive, openAd }) {
...
function AdDetailScreen({ ad, setActive, showToast, headerProps }) {
  const safeAd = ad || DEFAULT_ADS[0];

  return (
    <>
      <TopBar {...headerProps} back={() => setActive("feed")} title="Sponsored Ad" />
      <main className="space-y-5 px-4 pb-36 pt-[calc(env(safe-area-inset-top)+88px)]">
        <GlassCard className="overflow-hidden">
          <div className="relative h-64 bg-gradient-to-br from-[#ffdcc2] to-[#f8d8ff]">
            {safeAd.mediaUrl ? (
              safeAd.type === "video" ? (
                <video src={safeAd.mediaUrl} controls className="h-full w-full object-cover" />
              ) : (
                <img src={safeAd.mediaUrl} alt={safeAd.title} className="h-full w-full object-cover" />
              )
            ) : (
              <div className="grid h-full place-items-center">
                <KChatLogoMark size={86} />
              </div>
            )}
            <span className="absolute bottom-4 left-4 rounded-full bg-black/75 px-3 py-1 text-xs font-black text-white">
              {safeAd.price || "Sponsored"}
            </span>
          </div>

          <div className="p-5">
            <p className="text-xs font-black uppercase tracking-widest text-[#8f4e00]">
              {safeAd.business || "Sponsored"}
            </p>
            <h1 className="mt-2 text-2xl font-black leading-tight text-[#221a13]">{safeAd.title}</h1>
            <p className="mt-3 text-sm font-semibold leading-6 text-[#544437]">{safeAd.subtitle}</p>

            <div className="mt-4 flex items-center gap-2 text-xs font-black text-[#8f4e00]">
              <MapPin className="h-4 w-4" />
              {safeAd.location || "Imphal"}
            </div>

            <div className="mt-5 grid grid-cols-2 gap-3">
              <div className="rounded-[20px] bg-[#fff1e8] p-4">
                <div className="text-xl font-black text-[#8f4e00]">♡ {safeAd.likes || 0}</div>
                <div className="text-xs font-black uppercase text-[#877365]">Likes</div>
              </div>
              <div className="rounded-[20px] bg-[#e9fff9] p-4">
                <div className="text-xl font-black text-[#006b5e]">☏ {safeAd.comments || 0}</div>
                <div className="text-xs font-black uppercase text-[#877365]">Comments</div>
              </div>
            </div>

            <button
              onClick={() => showToast(`${safeAd.title} contact flow opened.`)}
              className="mt-5 w-full rounded-full bg-[#221a13] py-4 text-sm font-black text-white transition active:scale-95"
            >
              {safeAd.cta || "Open Ad"}
            </button>
          </div>
...
function AllAdsScreen({ ads = [], setActive, openAd, headerProps }) {
  const approvedAds = ads.filter((ad) => ad.status === "approved");
  const cards = approvedAds.length ? approvedAds : DEFAULT_ADS;

  return (
    <>
      <TopBar {...headerProps} back={() => setActive("feed")} title="All Sponsored Ads" />
      <main className="space-y-4 px-4 pb-36 pt-[calc(env(safe-area-inset-top)+88px)]">
        <div>
          <h1 className="text-2xl font-black text-[#221a13]">Local Spotlight Ads</h1>
          <p className="mt-1 text-sm font-semibold text-[#544437]">All approved sponsored posts in one place.</p>
        </div>

        <div className="space-y-3">
          {cards.map((ad) => (
            <GlassCard key={ad.id} className="overflow-hidden">
              <button onClick={() => openAd(ad)} className="flex w-full gap-3 p-3 text-left transition active:scale-[0.99]">
                <div className="grid h-24 w-24 shrink-0 place-items-center overflow-hidden rounded-[20px] bg-gradient-to-br from-[#ffdcc2] to-[#f8d8ff]">
                  {ad.mediaUrl ? (
                    ad.type === "video" ? (
                ...
                <div className="min-w-0 flex-1">
                  <span className="rounded-full bg-[#fff1e8] px-2 py-1 text-[10px] font-black uppercase text-[#8f4e00]">
                    {ad.price || "Sponsored"}
                  </span>
                  <h3 className="mt-2 line-clamp-1 font-black">{ad.title}</h3>
                  <p className="mt-1 line-clamp-2 text-xs font-semibold text-[#544437]">{ad.subtitle}</p>
                  <p className="mt-2 flex items-center gap-1 text-xs font-black text-[#8f4e00]">
                    <MapPin className="h-3 w-3" /> {ad.location || "Imphal"}
                  </p>
                </div>
              </button>
            </GlassCard>
